...
import { leadsApi, quotationsApi } from "@/lib/api";
import {
  BackendLead,
  BackendQuotation,
  LeadStatus,
...
export default function LeadsPage() {
  ...
  async function loadLeads() {
    ...
    setSelectedLeadId((current) =>
      current && leadsData.some((lead) => lead.id === current)
        ? current
        : leadsData[0]?.id || null,
    );
    setIsLoading(false);
  ...
  async function handleSaveLead(event: React.FormEvent<HTMLFormElement>) {
    ...
    setInfo(editingLeadId ? "Lead updated" : "Lead created");
    setIsSavingLead(false);
  }
  ...
  async function removeLead(leadId: string) {
    ...
    setError(null);
    setInfo(null);
    const response = await leadsApi.delete(leadId, user.id);
  ...
  async function createQuotation(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsCreatingQuote(false);
    await Promise.all([loadLeads(), loadQuotations(selectedLeadId)]);
  }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Create leads, move them to quotations, send quotes with PDF, and
            manage approvals.
          </p>
